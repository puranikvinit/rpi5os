...
void uart_init() {
  ...
  unsigned int uart_control = mmio_read_32(UART1_CR);
  uart_control &= ~(1); // Clear the UART enable[0] bit
  mmio_write_32(UART1_CR, uart_control);

  // Set the baud rate values
  uart_baud_rate_t uart_baud_rate = _calculate_baud_rate(9600);
  mmio_write_32(UART1_IBRD, uart_baud_rate.baud_rate_integer);
  mmio_write_32(UART1_FBRD, uart_baud_rate.baud_rate_fraction);

  // Set the line control register
  unsigned int uart_lcr_h = mmio_read_32(UART1_LCR_H);
  uart_lcr_h &= ~(7 << 4);
  uart_lcr_h |= (3 << 5); // 8-bit word length
  uart_lcr_h |= (1 << 4); // enable FIFO buffer
  mmio_write_32(UART1_LCR_H, uart_lcr_h);
...
void uart_putc(char c) {
  ...
  // while (mmio_read(UART1_FR) & (1 << 3))
  // ;
  // Wait until the TX FIFO is not full
  while (mmio_read_32(UART1_FR) & (1 << 5))
    ;

  mmio_write_32(UART1_DR, c);
}
...
char uart_getc() {
  ...
  while (mmio_read_32(UART1_FR) & (1 << 4))
    ;

  return (mmio_read_32(UART1_DR) & 0xff);
}
